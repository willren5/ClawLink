...
exports.ClawLinkError = exports.APP_ERROR_CODES = void 0;
exports.inferAppErrorCode = inferAppErrorCode;
exports.toClawLinkError = toClawLinkError;
exports.getAppErrorCode = getAppErrorCode;
exports.hasAppErrorCode = hasAppErrorCode;
...
class ClawLinkError extends Error {
    code;
    status;
    details;
    sourceCode;
...
exports.ClawLinkError = ClawLinkError;
...
function inferAppErrorCode(input) {
    ...
    if (includesAny(combined, ['pairing_required'])) {
        return exports.APP_ERROR_CODES.GATEWAY_PAIRING_REQUIRED;
    }
    if (includesAny(combined, ['device_identity_required'])) {
        return exports.APP_ERROR_CODES.GATEWAY_DEVICE_IDENTITY_REQUIRED;
    }
    if (includesAny(combined, ['origin not allowed'])) {
        return exports.APP_ERROR_CODES.GATEWAY_ORIGIN_NOT_ALLOWED;
    }
    if (includesAny(combined, ['too many requests', 'rate limit']) ||
        input.status === 429) {
        return exports.APP_ERROR_CODES.HTTP_RATE_LIMITED;
    }
    if (includesAny(combined, ['unsupported model', 'model was rejected']) ||
        (combined.includes('model') && includesAny(combined, ['invalid', 'not found', 'not available']))) {
        return exports.APP_ERROR_CODES.MODEL_UNSUPPORTED;
    }
    if (combined.includes('session') &&
        includesAny(combined, ['not found', 'unknown session'])) {
        return exports.APP_ERROR_CODES.SESSION_NOT_FOUND;
    }
    if (includesAny(combined, [
        ...
        return exports.APP_ERROR_CODES.REQUEST_INVALID;
    }
    if (includesAny(combined, ['html page', 'returned html', 'unexpected api response']) ||
        combined.includes('schema validation failed')) {
        return exports.APP_ERROR_CODES.RESPONSE_SCHEMA_INVALID;
    }
    if (includesAny(combined, ['timeout'])) {
        return exports.APP_ERROR_CODES.TIMEOUT;
    }
    if (includesAny(combined, [
...
function hasAppErrorCode(error, ...codes) {
